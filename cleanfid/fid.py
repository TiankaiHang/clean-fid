...
def fid_folder(fdir, dataset_name, dataset_res, dataset_split,
               ...
               custom_image_tranform=None, custom_fn_resize=None, stats_folder=None):
...
"""
def test_stats_exists(name, mode, stats_folder=None, model_name="inception_v3", metric="FID"):
...
"""
def remove_custom_stats(name, mode="clean", stats_folder=None, model_name="inception_v3"):
    if stats_folder is None:
        stats_folder = os.path.join(os.path.dirname(cleanfid.__file__), "stats")
...
def make_custom_stats(name, fdir, num=None, mode="clean", model_name="inception_v3",
                    stats_folder=None, num_workers=0, batch_size=64, 
                    device=torch.device("cuda"), verbose=True):
    if stats_folder is None:
        stats_folder = os.path.join(os.path.dirname(cleanfid.__file__), "stats")
...
def compute_kid(fdir1=None, fdir2=None, gen=None,
            ...
            verbose=True, use_dataparallel=True, stats_folder=None):
